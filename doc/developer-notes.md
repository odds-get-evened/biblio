...
- [Development guidelines](#development-guidelines)
    - [General biblio Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
biblio Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
biblio Core is a multi-threaded application, and deadlocks or other
...
biblio Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as biblio Core, where everyone uses
...
pay attention to for reviewers of biblio Core code.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: biblio Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of biblio Core, in which case
...
a biblio Core subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with biblio's `select()` loop, because
...
compatibility between nodes. This happened in biblio 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to biblio's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of biblio Core, the wallet was always
