...
It is possible to run biblio Core as a Tor onion service, and connect to such services.
...
- Starting with version 22.0, biblio Core only supports Tor version 3 hidden
  services (Tor v3). Tor v2 addresses are ignored by biblio Core and neither
  relayed nor stored.

- Tor removed v2 support beginning with version 0.4.6.

## How to see information about your Tor configuration via biblio Core

There are several ways to see your local onion address in biblio Core:
...
## 1. Run biblio Core behind a Tor proxy

The first step is running biblio Core behind a Tor proxy. This will already anonymize all
...
## 2. Automatically create a biblio Core onion service

biblio Core makes use of Tor's control socket API to create and destroy
ephemeral onion services programmatically. This means that if Tor is running and
proper authentication has been configured, biblio Core automatically creates an
onion service to listen on. The goal is to increase the number of available
onion nodes.

This feature is enabled by default if biblio Core is listening (`-listen`) and
...
## 3. Manually create a biblio Core onion service
...
- Do not add anything but biblio Core ports to the onion service created in section 3.
