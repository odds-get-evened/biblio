# User-space, Statically Defined Tracing (USDT) for biblio Core

biblio Core includes statically defined tracepoints to allow for more
...
**Note**: biblio Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to biblio Core
...
To list probes in biblio Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in biblio Core.
