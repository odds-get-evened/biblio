...
The data directory is the default location where the biblio Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.bitcoin/`
macOS    | `$HOME/Library/Application Support/biblio/`
Windows  | `%LOCALAPPDATA%\biblio\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual biblio blocks (dumped in network format, 128 MiB per file)
...
These subdirectories and files are no longer used by biblio Core:
