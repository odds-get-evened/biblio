# CJDNS support in biblio Core

It is possible to run biblio Core over CJDNS, an encrypted IPv6 network that
...
done outside of biblio Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the biblio network and individual nodes.
...
before it will work with your biblio Core node. This is described in steps
...
## Run biblio Core with CJDNS

Once you are connected to the CJDNS network, the following biblio Core
...
When enabled, this option tells biblio Core that it is running in an
...
IPv6 local network. This helps biblio Core perform better address management:
...
CJDNS support was added to biblio Core in version 23.0 and there may be fewer
...
## CJDNS-related information in biblio Core

There are several ways to see your CJDNS address in biblio Core:
