...
Currently, it is possible to create a multisig wallet using biblio Core only.
...
At the time of writing, there is no way to extract a specific path from wallets in biblio Core. For this, an external signer/xpub can be used.
...
Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, biblio Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in biblio Core will include the checksum in their output.
...
Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed biblio Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a biblio transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]
