biblio Core version 28.0 is now available from:
...
installer (on Windows) or just copy over `/Applications/biblio-Qt` (on macOS)
...
Running biblio Core binaries on macOS requires self signing.
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. biblio
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\biblio`
to `C:\Users\Username\AppData\Local\biblio`. biblio Core will check the existence
...
- Previously if biblio Core was listening for P2P connections, either using
...
- biblio Core will now fail to start up if any of its P2P binds fail, rather
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running biblio Core with the configuration
...
are now required to compile biblio Core. (#29091, #30263)

- The minimum required glibc to run biblio Core is now
...
Previous releases of biblio Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
