biblio Core version 0.19.0.1 is now available from:
...
installer (on Windows) or just copy over `/Applications/biblio-Qt` (on Mac)
or `bitcoind`/`bitcoin-qt` (on Linux).

Upgrading directly from a version of biblio Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of biblio Core are generally supported.

Compatibility
==============

biblio Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use biblio Core on unsupported systems.

biblio Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, biblio Core does not yet change appearance when
macOS "dark mode" is activated.

Users running macOS Catalina may need to "right-click" and then choose "Open"
to open the biblio Core .dmg. This is due to new signing requirements
imposed by Apple, which the biblio Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running biblio Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of biblio
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile biblio Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in biblio Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of biblio Core. (#14054)

- To eliminate well-known denial-of-service vectors in biblio Core,
  ...
  This prevents biblio Core from sending the BIP111 NODE_BLOOM service
  ...
  of biblio Core and nodes that have manually enabled BIP37 support,
...
- By default, biblio Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how biblio Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  biblio Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, biblio Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 biblio address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  LEX), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in LEX per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in biblio Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@biblio AEAD (jonasschnelli)
...
- #16521 Use the default maxfeerate value as LEX/kB (Remagpie)
...
- #15773 Add biblioTestFramework::sync_* methods (MarcoFalke)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding biblio Core (luke-jr)
...
- #16929 follow-up to rpc: default maxfeerate value as LEX/kB (jonatack)
