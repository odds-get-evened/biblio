biblio Core version 0.17.0 is now available from:
...
biblio Core is extensively tested on multiple operating systems using
...
BIP 174 Partially Signed biblio Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for biblio transactions that are not fully signed
...
Overall, the construction of a fully signed biblio transaction goes through the
following steps:
...
- An **Extractor** produces a valid biblio transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in LEX. This new field deprecates previous fee fields, such as
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in LEX (mryandao)
...
- #13528 `b877c39` Move biblioGUI initializers to class, fix initializer order warning (laanwj)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use biblioTestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use biblioTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use biblioTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use biblioTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for biblioTestFramework (WillAyd)
