biblio Core version 0.13.0 is now available from:
...
Various code modernizations have been done. The biblio Core code base has
...
HD wallets are incompatible with older versions of biblio Core.
...
proposed parameters, a future release of biblio Core is expected that
...
The two phases are distinct in the biblio-Qt GUI. During the first one,
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use LEX/kB (MarcoFalke)
...
- #8207 `f7a403b` Add a link to the biblio-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [biblio-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
