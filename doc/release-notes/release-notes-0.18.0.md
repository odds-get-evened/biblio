biblio Core version 0.18.0 is now available from:
...
`/Applications/biblio-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
biblio Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use biblio Core on unsupported systems.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the biblio Core
...
`~bitcoin/.bitcoin`. This change makes biblio Core more consistent with
...
- A new document introduces biblio Core's BIP174 [Partially-Signed
  biblio Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent biblio-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  biblio Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when biblio Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of biblio Core, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside biblio
...
This section describes planned changes to biblio Core that may affect
other biblio software and services.

- Since version 0.16.0, biblio Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with biblio Core 0.20 (expected about
  a year after 0.18), biblio Core will default to native segwit
  ...
  to bech32 addresses, and if the biblio Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in biblio Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any biblio Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, biblio Core
...
- A sub-project of biblio Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with biblio Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to biblio Core's own implementation, although entropy
  gathered by biblio Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves biblio Core a
...
- On macOS, biblio Core now opts out of application CPU throttling
...
- #11625 Add biblioApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for biblio Core (sipsorcery)
...
- #12775 Integration of property based testing into biblio Core (Christewart)
