...
// Copyright (c) 2009-2022 The biblio Core developers
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\biblio
    //   new: C:\Users\Username\AppData\Local\biblio
    // macOS: ~/Library/Application Support/biblio
    // Unix-like: ~/.bitcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "biblio";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "biblio";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/biblio";
#else
    // Unix-like
