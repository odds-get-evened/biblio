// Copyright (c) 2011-2022 The biblio Core developers
...
const std::string biblioGUI::DEFAULT_UIPLATFORM =
...
biblioGUI::biblioGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &biblioGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &biblioGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &biblioGUI::showModalOverlay);
...
void biblioGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a biblio address"));
    ...
    connect(overviewAction, &QAction::triggered, this, &biblioGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &biblioGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &biblioGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your biblio addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified biblio addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed biblio Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed biblio Transaction from clipboard"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible biblio command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &biblioGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &biblioGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &biblioGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &biblioGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &biblioGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &biblioGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &biblioGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &biblioGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &biblioGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &biblioGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &biblioGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &biblioGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &biblioGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &biblioGUI::showDebugWindow);
}

void biblioGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void biblioGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &biblioGUI::setCurrentWalletBySelectorIndex);
...
void biblioGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &biblioGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &biblioGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &biblioGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &biblioGUI::showProgress);
...
#ifdef ENABLE_WALLET
void biblioGUI::enableHistoryAction(bool privacy)
{
...
void biblioGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &biblioGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &biblioGUI::removeWallet);
...
WalletController* biblioGUI::getWalletController()
{
    return m_wallet_controller;
}

void biblioGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &biblioGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &biblioGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &biblioGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &biblioGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &biblioGUI::incomingTransaction);
    connect(this, &biblioGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void biblioGUI::removeWallet(WalletModel* walletModel)
{
...
void biblioGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void biblioGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void biblioGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void biblioGUI::setWalletActionsEnabled(bool enabled)
{
...
void biblioGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &biblioGUI::toggleHidden);
...
void biblioGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void biblioGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void biblioGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void biblioGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void biblioGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void biblioGUI::openClicked()
{
...
void biblioGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void biblioGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void biblioGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void biblioGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void biblioGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void biblioGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void biblioGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void biblioGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to biblio network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void biblioGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void biblioGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void biblioGUI::updateHeadersSyncProgressLabel()
{
...
void biblioGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void biblioGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &biblioGUI::quitRequested);
...
void biblioGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void biblioGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &biblioGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void biblioGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void biblioGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &biblioGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &biblioGUI::show);
                e->ignore();
            }
        }
    }
...
void biblioGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void biblioGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void biblioGUI::incomingTransaction(const QString& date, biblioUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(biblioUnits::formatWithUnit(unit, amount, true));
...
void biblioGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void biblioGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool biblioGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool biblioGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void biblioGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void biblioGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void biblioGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void biblioGUI::updateProxyIcon()
{
    std::string ip_port;
...
void biblioGUI::updateWindowTitle()
{
...
void biblioGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void biblioGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void biblioGUI::detectShutdown()
{
...
void biblioGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void biblioGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(biblioGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = biblioGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void biblioGUI::subscribeToCoreSignals()
{
...
void biblioGUI::unsubscribeFromCoreSignals()
{
...
bool biblioGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<biblioUnit> units = biblioUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const biblioUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, biblioUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const biblioUnit u : biblioUnits::availableUnits()) {
        menu->addAction(biblioUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(biblioUnit newUnits)
{
    setText(biblioUnits::longName(newUnits));
}
