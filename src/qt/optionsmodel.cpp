...
/** Call node.updateRwSetting() with biblio 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because biblio 22.x releases try to read these specific settings as
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplaybiblioUnit")) {
        settings.setValue("DisplaybiblioUnit", QVariant::fromValue(biblioUnit::LEX));
    }
    QVariant unit = settings.value("DisplaybiblioUnit");
    if (unit.canConvert<biblioUnit>()) {
        m_display_bitcoin_unit = unit.value<biblioUnit>();
    } else {
        m_display_bitcoin_unit = biblioUnit::LEX;
        settings.setValue("DisplaybiblioUnit", QVariant::fromValue(m_display_bitcoin_unit));
    }
...
{
    if (new_unit.isNull() || new_unit.value<biblioUnit>() == m_display_bitcoin_unit) return;
    m_display_bitcoin_unit = new_unit.value<biblioUnit>();
    QSettings settings;
    settings.setValue("DisplaybiblioUnit", QVariant::fromValue(m_display_bitcoin_unit));
